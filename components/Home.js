...
function Home() {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    let overview = data.overview.slice(0, 247) + '...';
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.original_title} overview={overview} poster={poster} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
